...
class Slider:
    ...
    def value(self):
        ...
        if mousePressed and dist(mouseX,mouseY,self.rectx,self.recty) < 20:
            self.rectx = mouseX
        #constrain rectangle
        self.rectx = float(constrain(self.rectx, self.x, self.x + 120))
        #draw rectangle
        strokeWeight(1)
        stroke(0)
        fill(255)
        rect(self.rectx,self.recty,10,20)
        self.val = float(map(self.rectx,self.x,self.x + 120,self.low,self.high))
        #draw label
        fill(0)
        textSize(10)
        text(float(self.val),self.rectx,self.recty + 35)
        #text label
